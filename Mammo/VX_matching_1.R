...
for (i in seq_along(df1_list)) {
  ...
  for (r in seq_len(nrow(df1))) {
    ...
    # Skip rows where patient_id or StudyDate is NA
    if (is.na(row1$patient_id) || is.na(row1$StudyDate)) next
    
    # Skip rows where patient_id, StudyDate, and AccessionNumber are all empty
    if (row1$patient_id == "" && row1$StudyDate == "" && row1$AccessionNumber == "") next
    ...
    if (nrow(match_rows) > 0) {
      ...
      # Check for patient_id mismatch:
      # Condition: same StudyDate and AccessionNumber but different patient_id
      if (row1$AccessionNumber != "") {
        cond_without_pid <- df2$StudyDate == row1$StudyDate & df2$AccessionNum == row1$AccessionNumber
        match_without_pid <- df2[cond_without_pid, , drop = FALSE]
      } else {
        match_without_pid <- df2[FALSE, , drop = FALSE]
      }
      
      patientid_mismatch <- FALSE
      if (nrow(match_without_pid) > 0) {
        if (match_without_pid$BDProjectID[1] != row1$patient_id) {
          patientid_mismatch <- TRUE
        }
      }
      
      # Check for AccessionNumber mismatch if AccessionNumber present
      if (row1$AccessionNumber != "") {
        cond_without_accession <- df2$BDProjectID == row1$patient_id & df2$StudyDate == row1$StudyDate
        match_without_accession <- df2[cond_without_accession, , drop = FALSE]
      } else {
        match_without_accession <- df2[FALSE, , drop = FALSE]
      }
      
      accession_mismatch <- FALSE
      if (nrow(match_without_accession) > 0) {
        if (match_without_accession$AccessionNum[1] != row1$AccessionNumber) {
          accession_mismatch <- TRUE
        }
      }
      
      # If neither patientid nor accession mismatch detected, record a general mismatch
      if (!patientid_mismatch && !accession_mismatch) {
        ...
        # If we have mismatches, record them
        if (accession_mismatch && nrow(match_without_accession) > 0) {
          mismatched_list[[length(mismatched_list) + 1]] <- data.frame(
            Mismatch_Reason = "AccessionNumber Mismatch",
            patient_id_file1 = row1$patient_id,
            patient_id_file2 = match_without_accession$BDProjectID[1],
            StudyDate_file1 = row1$StudyDate,
            StudyDate_file2 = match_without_accession$StudyDate[1],
            AccessionNumber_file1 = row1$AccessionNumber,
            AccessionNumber_file2 = match_without_accession$AccessionNum[1],
            InstitutionName_file1 = row1$InstitutionName,
            InstitutionName_file2 = match_without_accession$InstitutionName[1],
            Manufacturer_file1 = row1$Manufacturer,
            Sheet = sheet_name,
            stringsAsFactors = FALSE
          )
        }
        if (patientid_mismatch && nrow(match_without_pid) > 0) {
          mismatched_list[[length(mismatched_list) + 1]] <- data.frame(
            Mismatch_Reason = "PatientID Mismatch",
            patient_id_file1 = row1$patient_id,
            patient_id_file2 = match_without_pid$BDProjectID[1],
            StudyDate_file1 = row1$StudyDate,
            StudyDate_file2 = match_without_pid$StudyDate[1],
            AccessionNumber_file1 = row1$AccessionNumber,
            AccessionNumber_file2 = match_without_pid$AccessionNum[1],
            InstitutionName_file1 = row1$InstitutionName,
            InstitutionName_file2 = match_without_pid$InstitutionName[1],
            Manufacturer_file1 = row1$Manufacturer,
            Sheet = sheet_name,
            stringsAsFactors = FALSE
          )
